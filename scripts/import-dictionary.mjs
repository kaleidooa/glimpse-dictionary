...
const koRaw = await readFile(koPath, "utf8"),
  kengRaw = await readFile(kengPath, "utf8");
const normalize = (value) =>
  value.normalize("NFKC").trim().replace(/’/g, "'").toLowerCase();
const valid = (value) =>
  /^[a-z]+(?:['-][a-z]+)*$/.test(value) && value.length <= 80;
const clean = (value) =>
  value
    .replace(/<[^>]*>/g, "")
    .replace(/\s+/g, " ")
...
const sources = {
  wiktionary: {
    ...
    url: "https://kaikki.org/kowiktionary/%EC%98%81%EC%96%B4/index.html",
    sha256: createHash("sha256").update(koRaw).digest("hex"),
  },
  kengdic: {
    name: "Kengdic · Joe Speigle 및 기여자",
    license: "MPL-2.0",
    url: "https://github.com/garfieldnate/kengdic",
    sha256: createHash("sha256").update(kengRaw).digest("hex"),
  },
};
const wiki = new Map(),
  keng = new Map();
function add(map, word, entry) {
  ...
  if (!list.some((e) => e.pos === entry.pos && e.gloss === entry.gloss))
    list.push(entry);
  map.set(word, list);
}
for (const line of koRaw.trim().split("\n")) {
  ...
  if (
    item.lang_code !== "en" ||
    !valid(word) ||
    (/[A-Z]/.test(item.word[0]) && item.pos === "name")
  )
    continue;
  let pos = item.pos;
  if (pos === "unknown") {
    const category = item.categories?.map((c) => c.name).join(" ") ?? "";
    pos = /영어 형용사/.test(category)
      ? "adj"
      : /영어 부사/.test(category)
        ? "adv"
        : /영어 동사/.test(category)
  ...
  const ipa = item.sounds?.find(
    (s) =>
      typeof s.ipa === "string" &&
      !/^(?:en|US|UK)$/i.test(s.ipa) &&
      s.ipa.length < 90,
  )?.ipa;
...
for (const [name, map] of [
  ...
  counts[name] = { words: map.size, senses };
  bytes[name] = total;
}
...
const manifest = {
  ...
  bytes,
  changes:
    "English single-word entries only; whitespace/markup normalization; duplicate removal; Korean gloss filter; exact reversal of Kengdic glosses; alphabetical JSON shards. WordNet forms and Glimpse editorial entries are stored separately.",
};
await writeFile(
...
await copyFile(
  "scripts/import-dictionary.mjs",
  resolve(output, "import-dictionary.mjs"),
);
